...
func (l *GroupPutinLogic) GroupPutin(in *social.GroupPutinReq) (*social.GroupPutinResp, error) {
	...
	groupReq = &socialmodels.GroupRequests{
		...
		HandleResult: sql.NullInt64{
			Int64: int64(constant.NoHandlerResult),
			Valid: true,
		},
	}
	...
	if !groupInfo.IsVerify {
		...
		groupReq.HandleResult = sql.NullInt64{
			Int64: int64(constant.PassHandlerResult),
			Valid: true,
		}
	...
	// 验证进群方式
	if constant.GroupJoinSource(in.JoinSource) == constant.PutInGroupJoinSource {
		// 申请
	...
	if constant.GroupRoleLevel(inviteGroupMember.RoleLevel) == constant.CreatorGroupRoleLevel || constant.
		GroupRoleLevel(inviteGroupMember.RoleLevel) == constant.ManagerGroupRoleLevel {
		// 是管理者或创建者邀请
		defer createGroupMember()
